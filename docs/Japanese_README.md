...
| **BRK範囲** | 0x6000000000 ~ |
| **最大VMA** | 256エントリ |
| **機能** | mmap、munmap、mprotect、brk、ファイルバック mmap、msync |
| **File-backed** | `sys_mmap_file` はファイルを遅延マップ (page-fault で充填); MAP_SHARED のダーティページは munmap/msync で書き戻し |
...
| `dd if= of= [bs= count= skip= seek= conv=notrunc,fsync]` | ファイル・`/dev/zero`・生 `/dev/blkN` ノード間のブロックコピー |
...
| `nvidia gsp-bootargs` / `bootargs` | sysmem 内の libos/rmargs/CMDQ-MSGQ boot-args の構築+検証 (Falcon 起動なし) |
...
| `blkonline <drive>` | failfast で offline にされたデバイスを online に戻す (エラー連続をクリア) |
...
| `partprobe [drive]` | GPT を再読み込みし `/dev/blkNpM` ノードを実行時に更新 (partprobe(8)) |
...
#### GTX 1650 ドライバーモジュール (nvidia/gtx1650/)

| モジュール | 説明 |
|:--|:--|
| **gsprm.rs** | GSP-RM ステージング: VRAM プローブ、WPR2 レイアウト、radix3 ページテーブル、sysmem 確保 |
| **bootargs.rs** | GSP-RM ブート引数: libos init args、GSP_ARGUMENTS_CACHED、共有 CMDQ/MSGQ リージョン、FALCON_OS ハンドオフ |
| **msgq.rs** | CMDQ/MSGQ リングバッファ framing (host-producer + gsp-producer ページ) |
...
| 9 | wip | GSP-RM ステージング (`gsprm`) + フルブートオーケストレーター (`gsprm::boot`、`nvidia gsp-rm-boot-full`): scrub->load->ACR->WPR2->booter->MSGQ handshake。GSP-RM blob 埋め込み済み。ブート引数接続済み (`bootargs::GspBootArgs`): libos テーブル、共有 CMDQ/MSGQ、MAILBOX0/1 ハンドオフ、FALCON_OS = app_version。残りゲート 1 個: ACR の WPR2 ロック (SEC2 DMEM に `RM_FLCN_ACR_DESC` が必要) |
...
| **リトライ** | 一時的エラー (timeout/fault) は最大 2 回透過的に再発行; デバイスごとの errors/retries カウンター |
| **Failfast 状態** | デバイスごとに Online/Degraded/Offline (SCSI 方式); 連続 8 回の retry 後失敗でデバイスは Offline になり、タイムアウトでハングせず即座にエラーを返す。`blkonline <drive>` でリセット; 状態は `blkstat` と `/proc/diskstats` に表示 |
| **FUA バリア** | `block::write_barrier` は Force Unit Access (NVMe FUA ビット、ATA WRITE DMA FUA EXT) で ext3/4 ジャーナルの commit ブロックをコミット; FUA 非対応バックエンドは書き込み+flush にフォールバック |
| **partprobe** | `/dev/blkNpM` パーティションノードは実行時に GPT から再スキャン可能 (`partprobe`)、再起動不要 |
| **レイテンシ** | リクエストごとに TSC 計測; `blkstat` に平均 I/O レイテンシを表示 (iostat の await 相当) |
...
| パラメータ | 値 |
|:--|:--|
| **キュー** | アドミンキューペア (深さ16) + **4 I/O キューペア (深さ64)**、CPU 単位ルーティング、キューごとロックで複数 CPU が並列 submit/poll (blk-mq) |
