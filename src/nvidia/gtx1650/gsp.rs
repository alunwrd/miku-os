...
/// Halt-poll budget. The booter normally either runs to completion or
/// halts within a few thousand cycles; 10M spins is several hundred ms
/// even on a fast CPU and is more than enough
const HALT_SPIN_BUDGET: u32 = 10_000_000;
...
pub fn attempt_boot(bar0: &MmioRegion) -> Result<BootStatus, GspError> {
    ...
    let blob = tu116_fw::BOOTER_LOAD_570;
    ...
    // Clear mailboxes so we can reliably distinguish "booter wrote a status" from "we are reading stale POST values"
    ...
    if !gsp.wait_halted(HALT_SPIN_BUDGET) {
