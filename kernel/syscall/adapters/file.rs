...
use crate::syscall::errno::{err, vfs_err, EBADF, EFAULT, EINVAL, ENOENT, ENOMEM, ENOSPC, ENOSYS, EIO};
use crate::syscall::usercopy::{current_cr3, read_user_path, user_ptr_mapped, user_ptr_writable};
use crate::fs::vfs::types::{FileMode, OpenFlags, SeekFrom, VfsError, VNodeKind};
...
fn open_from_active_ext(path: &str, flags: OpenFlags, _mode: FileMode) -> Option<u64> {
    let info = crate::fs::ext::mount::with_ext2_pub(|fs| -> Result<_, VfsError> {
    ...
    Some(crate::fs::vfs::core::with_vfs(|vfs| {
        let id = match vfs.alloc_vnode() {
            Ok(id) => id,
            Err(e) => return vfs_err(e),
        };

        let ts = crate::fs::procfs::uptime_ticks();
        let name = path.rsplit('/').next().unwrap_or(path);
        vfs.nodes[id].init(
            id as crate::fs::vfs::InodeId,
            0,
            name,
            kind,
        ...
        if flags.has(OpenFlags::TRUNCATE) && flags.writable() && kind == VNodeKind::Regular {
            let truncate = crate::fs::ext::mount::with_ext2_pub(|fs| fs.ext3_truncate(ino));
        ...
        let fd = match vfs.fds().alloc(id as crate::fs::vfs::InodeId, flags) {
            Ok(fd) => fd,
            Err(e) => {
                vfs.nodes[id].active = false;
                return vfs_err(e);
...
pub fn sys_open(path_ptr: u64, path_len: u64, flags: u64, mode: u64) -> u64 {
    ...
    let vfs_result = crate::fs::vfs::core::with_vfs(|vfs| {
    ...
    match vfs_result {
        ...
        Err(_) => {
            ...
            let data = match crate::fs::vfs_read::read_file_or_solib(path_str) {
                Some(d) => d,
            ...
            crate::fs::vfs::core::with_vfs(|vfs| {
                ...
                if file_len > crate::fs::vfs::address_space::AddressSpace::max_size() as usize {
                    let _ = vfs.close(fd);
...
pub fn sys_close(fd: u64) -> u64 {
    // stdin/stdout/stderr have no backing entry until something redirects
    // them; closing the terminal itself stays a no-op. Closing a *redirected*
    // standard descriptor drops the redirection and restores the terminal,
    // which is how a shell puts its own stdio back after `cmd > file`
    if fd <= 2 {
        let redirected =
        ...
        return crate::fs::vfs::core::with_vfs(|vfs| match vfs.close(fd as usize) {
            Ok(()) => 0,
            Err(e) => vfs_err(e),
        });
    }
    ...
    if crate::net::socket::is_socket_fd(fd) {
        let pid = crate::syscall::usercopy::current_pid();
    ...
    crate::fs::vfs::core::with_vfs(|vfs| match vfs.close(fd as usize) {
        Ok(())  => 0,
        Err(e)  => vfs_err(e),
    })
}
...
pub fn sys_seek(fd: u64, offset: u64, whence: u64) -> u64 {
    ...
    crate::fs::vfs::core::with_vfs(|vfs| match vfs.seek(fd as usize, seek) {
        Ok(pos) => pos,
        Err(e)  => vfs_err(e),
    })
}
...
pub fn sys_fsize(fd: u64) -> u64 {
    crate::fs::vfs::core::with_vfs(|vfs| match vfs.fstat(fd as usize) {
        Ok(st) => st.size,
        Err(e) => vfs_err(e),
    })
}
...
pub fn sys_dup(fd: u64) -> u64 {
    crate::fs::vfs::core::with_vfs(|vfs| match vfs.dup(fd as usize) {
...
pub fn sys_dup2(old_fd: u64, new_fd: u64) -> u64 {
    ...
    crate::fs::vfs::core::with_vfs(|vfs| match vfs.dup_to(old_fd as usize, new_fd as usize) {
        Ok(fd) => fd as u64,
        Err(e) => vfs_err(e),
    })
}
...
pub fn sys_pipe(fds_ptr: u64) -> u64 {
    ...
    let (read_fd, write_fd) = match crate::fs::vfs::core::with_vfs(|vfs| -> Result<(usize, usize), u64> {
        ...
        // Mark it as an actual pipe before detaching the name
        if let Ok(vid) = vfs.resolve_path(0, pipe_name) {
            vfs.nodes[vid].kind = crate::fs::vfs::types::VNodeKind::Pipe;
            vfs.nodes[vid].pipe_read_pos = 0;
        }
...
pub fn sys_fsync(fd: u64) -> u64 {
    let valid = crate::fs::vfs::core::with_vfs(|vfs| vfs.fds().get(fd as usize).is_ok());
    if !valid { return err(EBADF); }

    let result = crate::fs::ext::mount::with_ext2_pub(|fs| fs.sync());
    match result {
        Some(Ok(()))  => 0,
        Some(Err(_))  => err(EIO),
