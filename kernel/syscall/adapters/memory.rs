...
use crate::syscall::errno::{err, EFAULT, EINVAL};
use crate::syscall::usercopy::{current_cr3, user_ptr_mapped, user_ptr_writable, USER_MAX};
use crate::mm::mmap;
...
pub fn sys_mmap_file(args_ptr: u64) -> u64 {
    ...
    let Some((vnode_id, file_size)) = crate::fs::vfs::core::fd_backing(fd) else {
        return err(EINVAL);
    };
...
// accessed via sched::current_cwd) up to the root vnode. Returns
...
pub fn sys_getcwd(buf: u64, size: u64) -> u64 {
    ...
    let cwd_id = crate::sched::current_cwd() as usize;
    let path = crate::fs::vfs::core::with_vfs(|vfs| {
        let mut cur = cwd_id;
...
pub fn sys_map_lib(name_ptr: u64, name_len: u64) -> u64 {
    ...
    match crate::process::solib::map_into_process(soname, cr3) {
        Ok(base) => base,
        Err(e)   => e as u64,
    }
}
