...
pub unsafe extern "C" fn schedule_from_isr(old_rsp: u64) -> u64 {
    ...
    // The percpu user_rsp slot (gs:[0x18], written by the syscall entry) is
    // per-CPU, not per-process. Swap it alongside kernel_rsp so a process
    // that blocked inside a syscall returns to ITS user stack even after
    // other processes ran syscalls on this CPU in between
    if !curr_ptr.is_null() {
        (*curr_ptr).saved_user_rsp.store(
            cpu.user_rsp.load(Ordering::Relaxed),
            Ordering::Relaxed,
        );
    }
    cpu.user_rsp.store(
        next.saved_user_rsp.load(Ordering::Relaxed),
        Ordering::Relaxed,
    );
    ...
    crate::arch::x86_64::gdt::set_kernel_stack(new_rsp0);
