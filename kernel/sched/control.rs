...
pub fn yield_now() {
    interrupts::without_interrupts(|| {
        ...
        let p = unsafe { &*ptr };
        // The idle task must never enter the run queue: it is the fallback
        // the scheduler picks when the queue is empty, and queueing it lets
        // pop_min hand the CPU back to idle while real work is waiting
        if p.state.load(Ordering::Relaxed) == STATE_RUNNING && !p.is_idle {
...
pub fn sleep(ticks: u64) {
    let wake_tick = crate::arch::x86_64::interrupts::get_tick() + ticks;
    interrupts::without_interrupts(|| {
        let cpu  = percpu::current();
        let curr = cpu.current_pid.load(Ordering::Relaxed);
        let ptr  = unsafe { PROC_INDEX.get_raw(curr) };
    ...
    interrupts::without_interrupts(|| {
        ...
        if target != percpu::current_index() {
            crate::arch::x86_64::apic::send_ipi(
                ...
                crate::arch::x86_64::apic::VEC_IPI_RESCHED,
            );
        }
    });
}
