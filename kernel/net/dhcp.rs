...
pub struct DhcpResult {
    ...
    pub dns: [u8; 4],
    pub server: [u8; 4],
    /// Option 51 lease time in seconds (0 if the server sent none)
    pub lease_secs: u32,
}
...
fn parse_offer(buf: &[u8], xid: u32) -> Option<DhcpResult> {
    ...
    Some(DhcpResult { ip: yiaddr, mask, gw, dns, server, lease_secs })
}

pub fn do_dhcp() -> Option<DhcpResult> {
    // Claim the DHCP client port for the whole exchange; replies are routed
    // to this queue by the RX demux
    let udp_h = match super::rx::register_udp(68) {
        Some(h) => h,
        None => {
            crate::serial_println!("[dhcp] udp port 68 busy");
            return None;
        }
    };
    let r = do_dhcp_inner(udp_h);
    super::rx::unregister_udp(udp_h);
    r
}

fn do_dhcp_inner(udp_h: usize) -> Option<DhcpResult> {
...
fn send_and_recv(
    ...
    loop {
        ...
        super::rx::pump();
        while let Some(dgram) = super::rx::udp_take(udp_h) {
            if let Some(r) = parse_offer(&dgram.data, xid) {
                return Some(r);
            }
        }
