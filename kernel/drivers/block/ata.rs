...
fn irq_flag(base_port: u16) -> &'static core::sync::atomic::AtomicBool {
    if base_port == 0x1F0 {
        &crate::arch::x86_64::interrupts::ATA_PRIMARY_IRQ
    } else {
        &crate::arch::x86_64::interrupts::ATA_SECONDARY_IRQ
    }
}
...
pub fn dma_init() {
    use crate::drivers::bus::pci::{pci_read32, pci_read8};
            ...
            for func in 0..8u8 {
                ...
                if class == 0x01 && subclass == 0x01 {
                    ...
                    if bar4 & 1 != 0 {
                        ...
                        unsafe {
                            Port::<u32>::new(crate::drivers::bus::pci::PCI_ADDR)
                                .write(crate::drivers::bus::pci::pci_addr(bus, dev, func, cmd_off));
                            Port::<u32>::new(crate::drivers::bus::pci::PCI_DATA)
                                .write(cmd | 0x0007);
                        }
...
use crate::io::block::driver::{BlkError, BlockDevInfo, BlockDriver, HealthInfo};
