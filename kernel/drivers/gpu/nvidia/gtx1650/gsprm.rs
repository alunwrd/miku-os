...
pub fn parse_gsp_rm(blob: &[u8]) -> Result<GspRmFw<'_>, ElfError> {
    parse_gsp_rm_sig(blob, GSP_SIGNATURE_SECTION_TU11X)
}

/// Same, but with a caller-chosen '.fwsignature_<grp>' section - the
/// gsp-<line>.bin container is shared across chip families and each family
/// carries its own signature section (Blackwell consumer: ".fwsignature_gb20x")
pub fn parse_gsp_rm_sig<'a>(blob: &'a [u8], sig_section: &'static str) -> Result<GspRmFw<'a>, ElfError> {
    if blob.is_empty() { return Err(ElfError::TooShort); }
    let img = find_section(blob, ".fwimage")?;
    let ver = find_section(blob, ".fwversion")?;
    let sig = find_section(blob, sig_section)?;
    Ok(GspRmFw {
...
pub fn boot(bar0: &MmioRegion, gpu: &crate::drivers::gpu::nvidia::pci::GpuDevice) -> FullBootReport {
    ...
    let (rom_src, rom_opt) = match crate::drivers::gpu::nvidia::vbios::read_rom_pramin(bar0) {
        Some(r) => ("PRAMIN", Some(r)),
        None => ("PROM", crate::drivers::gpu::nvidia::vbios::read_rom(gpu)),
    };
    match rom_opt {
        Some(rom) => {
