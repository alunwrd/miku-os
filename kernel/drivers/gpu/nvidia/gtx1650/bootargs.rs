...
/// GSP_ARGUMENTS_CACHED, r570 layout (rm/r570/nvrm/gsp.h), 72 bytes:
///   MESSAGE_QUEUE_INIT_ARGUMENTS (32): u64 sharedMemPhysAddr@0,
///     u32 pageTableEntryCount@8, (pad@12), u64 cmdQueueOffset@16,
///     u64 statQueueOffset@24 (the r535 lockless queue fields are gone)
///   GSP_SR_INIT_ARGUMENTS: oldLevel@32, flags@36, bInPMTransition@40
///   gpuInstance u32@44
///   bDmemStack u8@48 - r570_gsp_set_rmargs sets this to 1; libos in the
///     570.144 line runs its init task on a DMEM stack and misbehaves
///     without it
///   profilerArgs {u64 pa@56; u64 size@64}
fn write_rmargs(
    dst: &mut [u8],
    shared_mem_phys: u64,
    pte_count: u32,
    ...
    // srInitArguments / gpuInstance / profilerArgs stay zero for a cold
    // boot (buffer is pre-zeroed); bDmemStack must be 1 on r570
    dst[48] = 1;
}
...
impl GspBootArgs {
    ...
    // ------------------------------------------------------------------
    // Raw ring access for the post-boot RPC layer (gsp_common::rpc).
    // Pointer cross-wiring per nouveau r535_gsp_shared_init:
    //   host cmdq producer ptr   = cmdq page0 tx.writePtr   (@cmdq_off+16)
    //   GSP  cmdq consumer ptr   = msgq page0 rx.readPtr    (@msgq_off+32)
    //   GSP  msgq producer ptr   = msgq page0 tx.writePtr   (@msgq_off+16)
    //   host msgq consumer ptr   = cmdq page0 rx.readPtr    (@cmdq_off+32)
    // All pointers are in element pages (0..entries), elements start at
    // page 1 of each queue region
    // ------------------------------------------------------------------

    /// Ring depth in element pages (63 for the 256 KiB queues)
    pub fn queue_entries(&self) -> u32 { self.msg_count }

    fn shared_u32(&self, off: usize) -> u32 {
        let s = self.shared.as_slice();
        u32::from_le_bytes(s[off..off + 4].try_into().unwrap())
    }

    fn shared_set_u32(&mut self, off: usize, v: u32) {
        let s = self.shared.as_mut_slice();
        s[off..off + 4].copy_from_slice(&v.to_le_bytes());
    }

    /// Host cmdq producer pointer
    pub fn cmdq_write_ptr(&self) -> u32 { self.shared_u32(self.cmdq_off + 16) }

    /// Bump the cmdq producer pointer (with a write barrier so the framed
    /// element bytes are visible to GSP first)
    pub fn set_cmdq_write_ptr(&mut self, v: u32) {
        DmaBuffer::write_barrier();
        self.shared_set_u32(self.cmdq_off + 16, v);
        DmaBuffer::write_barrier();
    }

    /// GSP's consumer pointer into our cmdq
    pub fn cmdq_gsp_read_ptr(&self) -> u32 {
        core::sync::atomic::fence(core::sync::atomic::Ordering::Acquire);
        self.shared_u32(self.msgq_off + 32)
    }

    /// GSP's producer pointer into the msgq
    pub fn msgq_gsp_write_ptr(&self) -> u32 {
        core::sync::atomic::fence(core::sync::atomic::Ordering::Acquire);
        self.shared_u32(self.msgq_off + 16)
    }

    /// Our consumer pointer into the msgq
    pub fn msgq_host_read_ptr(&self) -> u32 { self.shared_u32(self.cmdq_off + 32) }

    pub fn set_msgq_host_read_ptr(&mut self, v: u32) {
        DmaBuffer::write_barrier();
        self.shared_set_u32(self.cmdq_off + 32, v);
        DmaBuffer::write_barrier();
    }

    /// One 4 KiB cmdq element page (idx in 0..entries), for the producer
    pub fn cmdq_page_mut(&mut self, idx: u32) -> &mut [u8] {
        let off = self.cmdq_off + ENTRY_OFF + (idx as usize % self.msg_count as usize) * GSP_PAGE;
        &mut self.shared.as_mut_slice()[off..off + GSP_PAGE]
    }

    /// One 4 KiB msgq element page (idx in 0..entries), for the consumer
    pub fn msgq_page(&self, idx: u32) -> &[u8] {
        let off = self.msgq_off + ENTRY_OFF + (idx as usize % self.msg_count as usize) * GSP_PAGE;
        &self.shared.as_slice()[off..off + GSP_PAGE]
    }

    /// GSP-RM log buffers, for post-mortem reading (libos writes its boot
    /// log into LOGINIT; RM messages land in LOGRM)
    pub fn loginit_slice(&self) -> &[u8] { self.loginit.as_slice() }
    pub fn logintr_slice(&self) -> &[u8] { self.logintr.as_slice() }
    pub fn logrm_slice(&self) -> &[u8] { self.logrm.as_slice() }
