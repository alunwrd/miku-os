...
/// Global handle for the first GB206 (RTX 5060 / 5060 Ti) we bring up
static ACTIVE_RTX5060: Mutex<Option<rtx5060::Rtx5060>> = Mutex::new(None);

pub fn set_active_rtx5060(dev: rtx5060::Rtx5060) {
    *ACTIVE_RTX5060.lock() = Some(dev);
}

/// Run a closure with the active GB206 device, if any
pub fn with_rtx5060<R>(f: impl FnOnce(&rtx5060::Rtx5060) -> R) -> Option<R> {
    let guard = ACTIVE_RTX5060.lock();
    guard.as_ref().map(f)
}
...
pub fn init() -> Result<(), &'static str> {
    ...
    for gpu in gpus.iter() {
        ...
        } else if rtx5060::matches(gpu.device) {
            // RTX 5060 / 5060 Ti (GB206, Blackwell): FSP-based boot path;
            // host bring-up + FSP transport + firmware staging probe
            if let Err(e) = rtx5060::init(gpu) {
                serial_println!("[nvidia] rtx5060 init failed: {}", e);
            }
        } else {
