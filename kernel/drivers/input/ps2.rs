...
/// Keyboard LED command (0xED + mask; bit0 scroll, bit1 num, bit2 caps)
const KBD_SET_LEDS: u8 = 0xED;

/// Set the keyboard LEDs. Works over both a real 8042 and USB legacy
/// emulation (same path the shell's keystrokes already take). Used as an
/// out-of-band status channel when the framebuffer console may be dead
/// (GSP-RM display init can kill GOP scanout). ACK bytes are drained
/// best-effort; a missing ACK is not an error worth failing on
pub fn set_leds(mask: u8) {
    let _ = write_data(KBD_SET_LEDS);
    crate::kcore::time::udelay(2_000);
    let _ = try_read_data(); // ACK
    let _ = write_data(mask & 0x07);
    crate::kcore::time::udelay(2_000);
    let _ = try_read_data(); // ACK
}

/// Blink all three LEDs 'times' times (~300 ms period), then leave them
/// in 'final_mask'. Diagnostic heartbeat visible without a working screen
pub fn blink_leds(times: u32, final_mask: u8) {
    for _ in 0..times {
        set_leds(0x07);
        crate::kcore::time::mdelay(150);
        set_leds(0x00);
        crate::kcore::time::mdelay(150);
    }
    set_leds(final_mask & 0x07);
}
