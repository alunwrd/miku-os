// process - process model, credentials/fd tables, signals, program
// execution (ELF load + exec), dynamic linking (dynlink/ldso/solib) and
// relocations. Core process logic lives directly in this module
...
pub struct Process {
    ...
    pub rsp:              AtomicU64,
    /// User-mode RSP captured by the syscall entry (percpu Cpu.user_rsp,
    /// gs:[0x18]). The percpu slot is one per CPU; when a process blocks
    /// inside a syscall and another process makes syscalls in between, the
    /// slot is overwritten. The scheduler saves/restores it here on every
    /// context switch so the syscall epilogue's 'mov rsp, gs:[0x18]'
    /// always returns to the right user stack
    ...
    /// Guard-page-fenced kernel stack (see mm::kstack)
    pub stack:           crate::mm::kstack::KernelStack,
    ...
    /// Userspace signal-dispatch entry point registered via sys_sigentry.
    /// 0 = none: fatal signals apply their default action in the kernel
    pub sig_entry:       AtomicU64,
    /// True between injecting a signal frame and sys_sigreturn; prevents
    /// nested delivery from trashing the interrupted context
    pub sig_in_delivery: AtomicBool,
    ...
    /// helper 'crate::sched::current_cwd()'
    pub cwd:         AtomicU64,
...
impl Process {
    pub fn alloc_raw(name: &'static str, priority: u8, cr3: u64) -> Box<Self> {
        ...
        let stack = crate::mm::kstack::KernelStack::new(DEFAULT_STACK_SIZE)
            .expect("out of physical memory allocating a kernel stack");
        Box::new(Self {
            ...
            sig_entry:        AtomicU64::new(0),
            sig_in_delivery:  AtomicBool::new(false),
    ...
    pub fn new_kernel_named(entry: fn() -> !, name: &'static str, priority: u8) -> Box<Self> {
        let cr3 = crate::mm::vmm::kernel_cr3();
    ...
    pub fn new_user_ring3(entry: u64, mut aspace: AddressSpace) -> Option<Box<Self>> {
        ...
        let stack_phys      = crate::mm::pmm::alloc_frames(USER_STACK_PAGES)?;
        ...
        if !aspace.map_range(stack_virt_base, stack_phys, stack_size, flags) {
            crate::mm::pmm::free_frames(stack_phys, USER_STACK_PAGES);
            return None;
        }
    ...
    pub fn new_fork(
        ...
        let mut p = Box::new(Self {
            ...
            wait_target:      AtomicU64::new(0),
            collected:        AtomicBool::new(false),
            cwd:              AtomicU64::new(crate::sched::cwd_of(parent_pid)),
            umask:            AtomicU16::new(crate::sched::umask_of(parent_pid)),
            uid:              AtomicU16::new(crate::sched::uid_of(parent_pid)),
            gid:              AtomicU16::new(crate::sched::gid_of(parent_pid)),
            euid:             AtomicU16::new(crate::sched::euid_of(parent_pid)),
            egid:             AtomicU16::new(crate::sched::egid_of(parent_pid)),
        });
        ...
        unsafe {
            ...
            f.add(16).write(crate::arch::x86_64::gdt::user_code_selector().0 as u64);
            f.add(17).write(user_rflags | 0x200); // ensure IF is set
            f.add(18).write(user_rsp);
            f.add(19).write(crate::arch::x86_64::gdt::user_data_selector().0 as u64);
        }
    ...
    pub fn cleanup_user_address_space(&mut self) {
        if self.cr3 == 0 || self.cr3 == crate::mm::vmm::kernel_cr3() { return; }
...
fn build_user_frame(kernel_stack_top: u64, rip: u64, user_rsp: u64) -> u64 {
    write_frame(
        ...
        crate::arch::x86_64::gdt::user_code_selector().0 as u64,
        user_rsp,
        crate::arch::x86_64::gdt::user_data_selector().0 as u64,
    )
}
