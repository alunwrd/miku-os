...
pub fn send_init_sipi(lapic_id: u32, start_page_phys: u32) {
    ...
    crate::kcore::time::udelay(10_000);
...
pub fn ioapic_init() -> Result<(), &'static str> {
    ...
    for info in &topo.ioapics {
        ...
        crate::mm::vmm::map_mmio_uc(info.addr, 0x1000);
...
// Installed from thread context (alloc_msi_vector) and read from the MSI
// IDT stubs, i.e. interrupt context - a plain spin::Mutex here deadlocks the
// CPU if the vector fires while its own handler slot is being written
static MSI_HANDLERS: [crate::kcore::irq_lock::IrqMutex<Option<fn()>>; VEC_MSI_COUNT] =
    [const { crate::kcore::irq_lock::IrqMutex::new(None) }; VEC_MSI_COUNT];
