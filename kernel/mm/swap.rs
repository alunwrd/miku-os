...
fn read_page(dev: BlockDevId, lba_base: u32, dst: *mut u8) -> Result<(), BlkError> {
    ...
    crate::io::block::read(dev, lba_base as u64, SWAP_SECS_PER_PAGE, buf)
}
...
fn write_page(dev: BlockDevId, lba_base: u32, src: *const u8) -> Result<(), BlkError> {
    ...
    crate::io::block::write(dev, lba_base as u64, SWAP_SECS_PER_PAGE, buf)
}
...
pub fn try_reclaim_page() -> Option<u64> {
    crate::mm::swap_map::alloc_or_evict()
}
...
pub fn swapon(
    ...
    crate::mm::pmm::refill_emergency_pool();
    crate::serial_println!(
        "[swap] emergency pool filled: {} frames ready",
        crate::mm::pmm::emergency_frames_available()
    );
