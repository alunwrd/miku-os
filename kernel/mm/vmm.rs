...
impl AddressSpace {
    ...
    pub fn free_address_space(&mut self) {
        ...
        let mut swap_guard = crate::mm::swap_map::lock_batch();
                ...
                for j in 0..512 {
                    ...
                    for k in 0..512 {
                        ...
                        for m in 0..512 {
                            ...
                            let pte = *raw;
                            if crate::mm::swap_map::is_swap_pte(pte) {
                                crate::mm::swap::free_swap_slot(crate::mm::swap_map::slot_from_pte(pte));
    ...
    pub fn map_page(&self, virt: u64, phys: u64, flags: PageTableFlags) -> bool {
        ...
        unsafe {
            ...
            crate::mm::swap_map::track(phys, self.cr3, virt, pinned);
        }
        true
    }
    ...
    pub fn unmap_page(&self, virt: u64) {
        ...
        unsafe {
            ...
            if crate::mm::swap_map::is_swap_pte(pte) {
                ...
                crate::mm::swap_map::untrack(phys);
                pmm::free_frame(phys);
            }
            *pte_ptr = 0;
    ...
    pub unsafe fn mark_swapped(&self, virt: u64, slot: u32) {
        let hhdm = grub::hhdm();
        let pte_val = crate::mm::swap_map::make_swap_pte(slot);
        unsafe {
            let Some(pte_ptr) = walk_to_pte(self.cr3, virt, hhdm) else { return; };
            *pte_ptr = pte_val;
            x86_64::instructions::tlb::flush(x86_64::VirtAddr::new(virt));
            ...
            for j in 0..512 {
                ...
                for k in 0..512 {
                    ...
                    for m in 0..512 {
                        ...
                        if crate::mm::swap_map::is_swap_pte(pte_val) {
                            continue;
                        }
...
/// The kernel's own page-table root, latched once at boot. Reading CR3 at
/// call time (the old behaviour) silently returned the CURRENT process's
/// root, which made every `cr3 == kernel_cr3()` guard trivially true from
/// a user context: fork returned EPERM for every process and exec leaked
/// the old address space
static KERNEL_CR3: core::sync::atomic::AtomicU64 = core::sync::atomic::AtomicU64::new(0);

/// Latch the running page-table root as "the kernel's". Must be called
/// during boot, before the first user process exists (kernel_main does)
pub fn latch_kernel_cr3() {
...
pub fn kernel_cr3() -> u64 {
    let v = KERNEL_CR3.load(core::sync::atomic::Ordering::Relaxed);
    if v != 0 {
        return v;
    }
    // Not latched yet: only possible during early boot, where the running
    // root IS the kernel's. Latch it now
    latch_kernel_cr3();
    KERNEL_CR3.load(core::sync::atomic::Ordering::Relaxed)
}

/// Break copy-on-write for one user page so it becomes privately writable.
/// Shared logic between the #PF handler (user write fault) and syscalls
/// that write out-parameters into user memory: after fork the parent's
/// stack pages are COW = read-only, and a kernel-side write must perform
/// the same copy-or-unshare the fault handler would. Returns false only
/// on allocation failure
pub fn break_cow_page(cr3: u64, va: u64) -> bool {
    let page_addr = va & !0xFFF;
    let pte_raw = match read_pte_raw(cr3, page_addr) {
        Some(p) => p,
        None => return false,
    };
    if pte_raw & PTE_PRESENT == 0 || pte_raw & PTE_COW == 0 {
        // nothing to break (either unmapped, or already private/writable)
        return true;
    }

    let old_phys = pte_raw & PTE_ADDR_MASK;
    let hhdm = grub::hhdm();
    let rc = pmm::ref_get(old_phys);
    ...
    let ok = if rc > 1 {
        match pmm::alloc_frame() {
            Some(new_phys) => {
                ...
                let remaining = pmm::ref_dec(old_phys);
                if remaining <= 1 {
                    crate::mm::swap_map::set_pinned(old_phys, false);
                }
                let new_pte = (new_phys & PTE_ADDR_MASK)
                    | (pte_raw & !PTE_ADDR_MASK & !PTE_COW)
                    | PTE_WRITABLE;
        ...
        // last reference: just drop the COW bit and make it writable
        let new_pte = (pte_raw & !PTE_COW) | PTE_WRITABLE;
        unsafe { aspace.write_pte_raw(page_addr, new_pte); }
        crate::mm::swap_map::set_pinned(old_phys, false);
        x86_64::instructions::tlb::flush(x86_64::VirtAddr::new(page_addr));
        true
...
pub unsafe fn mark_swapped_with_flags(cr3: u64, virt: u64, slot: u32, pte_flags: u64) {
    let hhdm = grub::hhdm();
    let aspace = AddressSpace::from_raw(cr3);
    let pte_val = crate::mm::swap_map::make_swap_pte_with_flags(slot, pte_flags);
    unsafe {
...
/// Walk the HHDM mapping down to the 4 KiB leaf covering `phys`, splitting
/// any huge page in the way, and hand back a pointer to that leaf entry.
///
/// The HHDM lives under PML4 slot 256, which every address space copies
/// verbatim from the kernel tables (`AddressSpace::new_user`). The copy is of
/// the P4 entry itself, so a change made further down this walk is visible in
/// every address space without touching them individually
unsafe fn hhdm_leaf_pte(phys: u64) -> Option<*mut x86_64::structures::paging::page_table::PageTableEntry> {
    let hhdm = grub::hhdm();
    let virt = (phys & !0xFFF) + hhdm;
    ...
    Some(&mut p1[p1_idx] as *mut _)
}

/// Make one physical frame unreachable through the HHDM.
///
/// Used to fence off kernel stacks: the frame below a stack is left mapped
/// nowhere, so running off the bottom raises a page fault at the offending
/// instruction instead of quietly scribbling over whatever the allocator
/// handed out next. Returns false if the HHDM has no mapping to clear
pub fn hhdm_unmap_guard_frame(phys: u64) -> bool {
    unsafe {
        let Some(pte) = hhdm_leaf_pte(phys) else { return false };
        (*pte).set_unused();
        core::arch::asm!("mfence", options(nostack, nomem));
        x86_64::instructions::tlb::flush_all();
    }
    crate::arch::x86_64::apic::broadcast_tlb_flush();
    true
}

/// Undo `hhdm_unmap_guard_frame`, restoring the plain writable HHDM mapping.
/// Must run before the frame goes back to the allocator, otherwise its next
/// owner faults the first time anything touches it through the HHDM
pub fn hhdm_remap_guard_frame(phys: u64) -> bool {
    unsafe {
        let Some(pte) = hhdm_leaf_pte(phys) else { return false };
        (*pte).set_addr(
