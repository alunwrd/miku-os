...
impl MikuVFS {
    pub(super) fn ext2_lazy_lookup(
        ...
        if !self.ext2_mount_active || !crate::fs::ext::mount::is_ext2_ready() {
    ...
    pub fn ext2_ensure_children_loaded(&mut self, dir_vnode: usize) -> VfsResult<()> {
        ...
        let result = crate::fs::ext::mount::with_ext2_pub(|fs| {
            use crate::fs::ext::structs::FT_SYMLINK;

            let inode = fs.read_inode(ext2_ino).map_err(|_| VfsError::IoError)?;
            let mut entries = [const { crate::fs::ext::structs::DirEntry::empty() }; 256];
            let count = fs
        ...
        for i in 0..child_count {
            ...
            use crate::fs::ext::structs::{FT_DIR, FT_SYMLINK};
    ...
    pub(super) fn write_ext2_file(
        ...
        if is_sync {
            let _ = crate::fs::ext::mount::with_ext2_pub(|fs| fs.sync());
