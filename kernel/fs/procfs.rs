...
/// Kept as the historical entry point; the clock itself lives in
/// kcore::clock, which anchors the epoch to a tick so the value actually
/// advances. The old version stored a bare number that nothing incremented,
/// so every timestamp after the NTP reply was identical
pub fn set_wall_clock(unix_secs: u64) {
    crate::kcore::clock::set_epoch(unix_secs, "ntp");
    WALL_CLOCK.store(unix_secs, Ordering::Relaxed);
}

pub fn wall_clock() -> u64 {
    crate::kcore::clock::now_unix()
}
...
fn format_meminfo(
    ...
    let heap_used = crate::mm::allocator::used();
    let heap_free = crate::mm::allocator::free();
    let heap_total = crate::mm::allocator::HEAP_SIZE;
...
fn format_stat(buf: &mut [u8; 192]) -> usize {
    ...
    pos += write_str(buf, pos, "\nheap_kb: ");
    pos += write_u64(buf, pos, (crate::mm::allocator::HEAP_SIZE / 1024) as u64);
    pos += write_str(buf, pos, "\n");
    pos
}

fn format_heap(buf: &mut [u8; 192]) -> usize {
    let used = crate::mm::allocator::used();
    let free = crate::mm::allocator::free();
    let total = crate::mm::allocator::HEAP_SIZE;
...
fn format_diskstats(buf: &mut [u8; 1024]) -> usize {
    ...
    for id in 0..crate::fs::vfs::types::MAX_BLOCK_DEVICES as u8 {
        ...
        let state = match st.state {
            crate::io::block::DevState::Online   => "online",
            crate::io::block::DevState::Degraded => "degraded",
            crate::io::block::DevState::Offline  => "offline",
        };
        let _ = writeln!(
            w,
            "blk{} {} {} {} {} {} {} {} {} {}",
