...
fn write_protective_mbr(dev: BlockDevId, total_sectors: u64) -> Result<(), BlkError> {
    ...
    mbr[510] = 0x55;
    mbr[511] = 0xAA;
    crate::io::block::write_sync(dev, 0, 1, &mbr)
}
    ...
    -> Result<(), BlkError>
{
    ...
    for chunk in 0..32u64 {
        ...
        crate::io::block::write_sync(dev, base_lba + chunk, 1, &buf)?;
    }
    Ok(())
}
...
fn write_full_table(dev: BlockDevId, tbl: &mut GptTable) -> Result<(), BlkError> {
    ...
    crate::io::block::write_sync(dev, GPT_HEADER_LBA as u64, 1, &hbuf)?;
    ...
    crate::io::block::write_sync(dev, tbl.total_sectors - 1, 1, &bbuf)?;
...
pub fn gpt_read(dev: BlockDevId) -> Result<GptTable, GptReadError> {
    let mut buf = [0u8; 512];
    crate::io::block::read(dev, GPT_HEADER_LBA as u64, 1, &mut buf).map_err(GptReadError::Io)?;
    ...
    let per_sec = 512 / GPT_ENTRY_SIZE;
    for chunk in 0..32u64 {
        crate::io::block::read(dev, GPT_ENTRIES_LBA as u64 + chunk, 1, &mut buf)
...
pub fn gpt_probe_sectors(dev: BlockDevId) -> u64 {
    ...
    if let Some(info) = crate::io::block::info(dev) {
    ...
    // readable LBA
    let mut buf = [0u8; 512];
    if crate::io::block::read(dev, 0, 1, &mut buf).is_err() { return 0; }
    let mut lo: u64 = 2048;
    let mut hi: u64 = (u32::MAX / 2) as u64;
    while hi > lo && crate::io::block::read(dev, hi - 1, 1, &mut buf).is_err() { hi /= 2; }
    while lo + 1 < hi {
        let mid = lo + (hi - lo) / 2;
        if crate::io::block::read(dev, mid, 1, &mut buf).is_ok() { lo = mid; } else { hi = mid; }
    }
    lo + 1
}
