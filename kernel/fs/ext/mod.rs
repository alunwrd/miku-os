...
pub struct MikuFS {
    ...
    pub group_count: u32,
    // Grown to group_count at mount time. This used to be a fixed
    // [GroupDesc; 32], which silently capped a mountable volume at 32 block
    // groups: 4 GiB with 4 KiB blocks, 256 MiB with 1 KiB blocks. Anything
    // larger was rejected with "too many block groups"
...
impl MikuFS {
    ...
    pub fn flush_group_desc(&mut self, group: usize) -> Result<(), FsError> {
        if group < self.groups_dirty.len() {
    ...
    pub fn flush_all_dirty_metadata(&mut self) -> Result<(), FsError> {
        ...
        for group in 0..self.groups_dirty.len() {
    ...
    pub fn get_timestamp(&self) -> u32 {
        let wall = crate::fs::procfs::wall_clock();
        if wall > 0 {
            wall as u32
        } else {
            (crate::fs::procfs::uptime_ticks() / crate::arch::x86_64::interrupts::PIT_HZ as u64) as u32
        }
    }
