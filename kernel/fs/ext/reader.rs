...
/// ('crate::io::block') rather than touching a driver directly
...
impl DiskReader {
    pub fn new(drive: AtaDrive) -> Self {
        let dev_id = crate::io::block::register_ata(drive);
    ...
    pub fn read_sector(&mut self, lba: u32, buf: &mut [u8; 512]) -> Result<(), FsError> {
        self.io_count += 1;
        crate::io::block::read(self.dev_id, (self.start_lba + lba) as u64, 1, buf)
            .map_err(|_| FsError::IoError)
    }

    pub fn write_sector(&mut self, lba: u32, buf: &[u8; 512]) -> Result<(), FsError> {
        self.io_count += 1;
        crate::io::block::write(self.dev_id, (self.start_lba + lba) as u64, 1, buf)
            .map_err(|_| FsError::IoError)
    }
    ...
    pub fn write_sector_sync(&mut self, lba: u32, buf: &[u8; 512]) -> Result<(), FsError> {
        self.io_count += 1;
        crate::io::block::write_sync(self.dev_id, (self.start_lba + lba) as u64, 1, buf)
            .map_err(|_| FsError::IoError)
    }
    ...
    pub fn write_block_sync(
        ...
        self.io_count += 1;
        crate::io::block::write_sync(self.dev_id, (self.start_lba + lba) as u64, sectors as u32, buf)
            .map_err(|_| FsError::IoError)
    }
    ...
    pub fn write_block_barrier(
        ...
        self.io_count += 1;
        crate::io::block::write_barrier(self.dev_id, (self.start_lba + lba) as u64, sectors as u32, buf)
            .map_err(|_| FsError::IoError)
    }
    ...
    pub fn read_block(
        ...
        self.io_count += 1;
        crate::io::block::read(self.dev_id, (self.start_lba + lba) as u64, sectors as u32, buf)
            .map_err(|_| FsError::IoError)
    }
    ...
    pub fn write_block(
        ...
        self.io_count += 1;
        crate::io::block::write(self.dev_id, (self.start_lba + lba) as u64, sectors as u32, buf)
            .map_err(|_| FsError::IoError)
    }

    pub fn flush_drive(&mut self) {
        self.io_count += 1;
        let _ = crate::io::block::flush(self.dev_id);
    }
