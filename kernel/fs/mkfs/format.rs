use crate::io::block::driver::BlkError;
use crate::fs::vfs::types::BlockDevId;
use super::layout::{FsLayout, group_has_sb};
use super::params::{FsType, MkfsParams};
use crate::fs::ext::structs::*;
use crate::fs::ext::ext3::journal::{JBD_MAGIC, JBD_SUPERBLOCK_V2};
...
impl Writer {
    fn write_sector(&mut self, lba: u32, buf: &[u8; 512]) -> Result<(), MkfsError> {
        crate::io::block::write(self.dev, (self.start_lba + lba) as u64, 1, buf)
    ...
    fn probe_sectors(&mut self) -> u32 {
        ...
        if let Some(info) = crate::io::block::info(self.dev) {
        ...
        let mut buf = [0u8; 512];
        if crate::io::block::read(self.dev, 0, 1, &mut buf).is_err() {
            return 0;
        }
        let mut lo: u32 = 1;
        let mut hi: u32 = u32::MAX / 2;
        while hi > lo && crate::io::block::read(self.dev, (hi - 1) as u64, 1, &mut buf).is_err() {
            hi /= 2;
            if hi == 0 { return lo; }
        }
        while lo + 1 < hi {
            let mid = lo + (hi - lo) / 2;
            if crate::io::block::read(self.dev, mid as u64, 1, &mut buf).is_ok() { lo = mid; }
            else { hi = mid; }
        }
        lo + 1
    }
...
pub fn mkfs(dev: BlockDevId, params: &MkfsParams) -> Result<MkfsReport, MkfsError> {
    ...
    if crate::io::block::info(dev).map_or(false, |i| i.discard) {
        ...
        while done < total_sectors as u64 {
            ...
            if crate::io::block::discard(dev, params.start_lba as u64 + done, n).is_err() {
    ...
    w.zero_sector(2)?;
    w.zero_sector(3)?;
    crate::io::block::flush(w.dev).map_err(MkfsError::Io)?;
    ...
    let now  = (crate::fs::procfs::uptime_ticks() / 18) as u32;
...
fn write_raw_inode(
    ...
    while rem > 0 {
        ...
        let mut sec = [0u8; 512];
        crate::io::block::read(w.dev, (w.start_lba + cur_lba) as u64, 1, &mut sec).map_err(MkfsError::Io)?;
