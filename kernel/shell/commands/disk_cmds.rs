...
fn blk_dev(idx: usize) -> crate::fs::vfs::types::BlockDevId {
    if idx < 4 {
...
pub fn cmd_blkstat() {
    ...
    for idx in 0..4 {
        let _ = crate::io::block::register_ata(AtaDrive::from_idx(idx));
    }
    crate::io::block::probe();

    let mut any = false;
    for id in 0..crate::fs::vfs::types::MAX_BLOCK_DEVICES as u8 {
        if let Some(info) = crate::io::block::info(id) {
            if info.total_sectors == 0 && info.model_len == 0 {
                continue;
            }
            any = true;
            let mb = info.total_sectors * info.sector_size as u64 / (1024 * 1024);
            let Some(st) = crate::io::block::dev_stats(id) else { continue };
            let kind_str = match st.kind {
                crate::io::block::DevKind::Ata       => "ata",
                crate::io::block::DevKind::VirtioBlk => "virtio",
                crate::io::block::DevKind::Ahci      => "ahci",
                crate::io::block::DevKind::Nvme      => "nvme",
                crate::io::block::DevKind::Ramdisk   => "ramdisk",
            };
            println!(
            ...
            match st.state {
                crate::io::block::DevState::Online => {}
                crate::io::block::DevState::Degraded => {
                    print_warn!("        state: DEGRADED (recent I/O errors)");
                }
                crate::io::block::DevState::Offline => {
                    print_error!(
    ...
    let (sub, done, err) = crate::io::block::io_stats();
    println!("  bio queue: submitted={} completed={} errors={}", sub, done, err);

    let (hits, misses, ra, merges, dirty) = crate::io::block::cache_stats();
...
fn nvme_worker() -> ! {
    ...
    'outer: for it in 0..120u32 {
        ...
        if crate::io::block::write_sync(dev, base, 8, &wbuf).is_err() { ok = false; break; }
        if crate::io::block::read(dev, base, 8, &mut rbuf).is_err() { ok = false; break; }
        for &b in rbuf.iter() {
            if b != pat { ok = false; break 'outer; }
        }
    }
    NS_RESULT[id].store(if ok { 1 } else { 2 }, Relaxed);
    loop { crate::sched::sleep(100_000); }
}
...
pub fn cmd_nvmestress() {
    ...
    let mut dev = None;
    for id in 0..crate::fs::vfs::types::MAX_BLOCK_DEVICES as u8 {
        if let Some(st) = crate::io::block::dev_stats(id) {
            if st.kind == crate::io::block::DevKind::Nvme {
                dev = Some(id);
                break;
            }
        }
    ...
    for p in pids.iter_mut() {
        *p = crate::sched::spawn(nvme_worker);
    }
    ...
    if let Some(st) = crate::io::block::dev_stats(d) {
...
pub fn cmd_partprobe(args: &str) {
    ...
    if arg.is_empty() {
        ...
        let mut any = false;
        for id in 0..crate::fs::vfs::types::MAX_BLOCK_DEVICES as u8 {
            let Some(info) = crate::io::block::info(id) else { continue };
            if info.total_sectors == 0 { continue; }
            any = true;
            let (now, removed) = crate::fs::vfs::core::rescan_block_partitions(id);
    ...
    let dev = blk_dev(idx);
    if crate::io::block::info(dev).map_or(true, |i| i.total_sectors == 0) {
        print_error!("  no device blk{}", idx);
        return;
    }
    let (now, removed) = crate::fs::vfs::core::rescan_block_partitions(dev);
...
pub fn cmd_blkonline(drive_str: &str) {
    ...
    let dev = blk_dev(idx);
    match crate::io::block::reset_device(dev) {
        None => print_error!("  no device blk{}", idx),
        Some(crate::io::block::DevState::Offline) => {
            print_success!("  blk{}: offline -> online, I/O will be retried", idx);
        }
        Some(crate::io::block::DevState::Degraded) => {
            print_success!("  blk{}: degraded -> online, error run cleared", idx);
        }
        Some(crate::io::block::DevState::Online) => {
...
pub fn cmd_blkzero(args: &str) {
    ...
    let dev = blk_dev(idx);
    if crate::io::block::info(dev).is_none() {
        print_error!("  no device blk{}", idx);
        return;
    }
    ...
    while done < count {
        ...
        match crate::io::block::write_zeroes(dev, lba + done, n) {
            Ok(()) => done += n as u64,
            Err(e) => {
...
pub fn cmd_fstrim() {
    enum Outcome {
        Unsupported(crate::fs::vfs::types::BlockDevId),
        Failed(crate::fs::vfs::types::BlockDevId, crate::fs::ext::FsError),
        Done(crate::fs::vfs::types::BlockDevId, crate::fs::ext::trim::TrimReport),
    }

    let res = crate::shell::commands::ext2_cmds::with_ext2_pub(|fs| {
        let dev = fs.reader.dev_id;
        if !crate::io::block::info(dev).map_or(false, |i| i.discard) {
...
pub fn cmd_blkdiscard(args: &str) {
    ...
    let dev = blk_dev(idx);
    let Some(info) = crate::io::block::info(dev) else {
        print_error!("  no device blk{}", idx);
        return;
    };
    if !info.discard {
    ...
    while done < sectors {
        ...
        match crate::io::block::discard(dev, lba + done, n) {
            Ok(()) => done += n as u64,
            Err(e) => {
...
pub fn cmd_smart(drive_str: &str) {
    ...
    match crate::io::block::health(dev) {
...
pub fn cmd_swapon_auto() {
    ...
    for drive_idx in 0..4usize {
        ...
        if crate::io::block::read(dev, 0, 1, &mut probe).is_err() {
            continue;
        }
