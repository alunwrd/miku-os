...
use crate::fs::vfs::with_vfs_ro;
use crate::mm::allocator;
use crate::io::console;
use crate::{cprint, cprintln, print, print_info, print_success, println};
...
pub fn cmd_info() {
    ...
    let ticks = crate::fs::procfs::uptime_ticks();
    ...
    let (pmm_used, pmm_total) = crate::mm::pmm::stats();
    ...
    cprintln!(57, 197, 187,  "  MikuOS v{}", env!("CARGO_PKG_VERSION"));
    cprintln!(230, 240, 240, "  VNodes: {}/{}", vn, crate::fs::vfs::MAX_VNODES);
    ...
    if crate::mm::swap::swap_is_active() {
        let stotal = crate::mm::swap::swap_total_kb();
        let sused  = crate::mm::swap::swap_used_kb();
        let sfree  = crate::mm::swap::swap_free_kb();
...
pub fn cmd_help() {
    ...
    cprintln!(128, 222, 217, "  ext2mount                mount an ext2/ext3/ext4 disk");
    ...
    cprintln!(128, 222, 217, "  mount ext<2|3|4> <path>  mount an ext volume at path");
    ...
    cprintln!(128, 222, 217, "  nvidia | gpu [sub]      NVIDIA GPU (info|debug|firmware|falcon|gsp|next)");
    cprintln!(128, 222, 217, "  usb | lsusb             xHCI controllers and USB devices");
...
pub fn cmd_ps() {
    let stats    = crate::sched::get_stats();
    let total_sw = crate::sched::total_switches();
    let now      = crate::arch::x86_64::interrupts::get_tick();
    ...
    let (pu, pt) = crate::mm::pmm::stats();
    let hk = crate::mm::allocator::used() / 1024;
...
pub fn cmd_nice(pid_str: &str, prio_str: &str) {
    ...
    crate::sched::set_priority(pid, prio);
...
pub fn cmd_affinity(pid_str: &str, mask_str: &str) {
    ...
    crate::sched::set_affinity(pid, mask);
...
pub fn cmd_usb() {
    cprintln!(57, 197, 187, "  USB (xHCI)");
    let lines = crate::drivers::input::usb::status_lines();
    if lines.is_empty() {
        println!("  usbd has not reported yet (still starting, or no controller)");
        return;
    }
    for line in lines {
        println!("  {}", line);
