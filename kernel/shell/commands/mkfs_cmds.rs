...
use crate::fs::mkfs::{mkfs, FsType, MkfsError, MkfsParams};
use crate::fs::mkfs::layout::FsLayout;
...
fn dev_from_index(i: usize) -> Option<BlockDevId> {
    match i {
        0..=3 => Some(crate::io::block::register_ata(AtaDrive::from_idx(i))),
        4..=7 => {
...
fn do_mkfs(args: &str, fs_type: FsType) {
    ...
    crate::shell::commands::ext2_cmds::invalidate_drive_mounts(drive_idx, params.start_lba);
...
fn probe_sectors(dev: BlockDevId) -> u32 {
    crate::fs::gpt::gpt_probe_sectors(dev).min(u32::MAX as u64) as u32
}
