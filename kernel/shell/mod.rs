...
use crate::{cprint, cprintln, print, serial_println};
...
/// Drop the session back to / after an ext subtree is unmounted - the cwd
/// we were holding may no longer exist. Registered with fs::ext::mount so
/// the filesystem can announce the event without knowing about the shell
fn on_ext_unmount() {
...
pub fn init() {
    crate::fs::ext::mount::set_unmount_hook(on_ext_unmount);
...
pub fn kbd_thread() -> ! {
    ...
    loop {
        if !crate::kcore::boot_state::is_done() {
            crate::sched::sleep(CMD_POLL_TICKS);
            continue;
        }
        let mut got = false;
        while let Some(sc) = crate::io::input::stdin::pop() {
            got = true;
            if let Ok(Some(ev)) = keyboard.add_byte(sc) {
                if let Some(key) = keyboard.process_keyevent(ev) {
                    if crate::io::input::user_stdin::is_foreground_active() {
                        match key {
                            DecodedKey::Unicode('\u{0003}') => {
                                // Ctrl+C -> SIGINT to the foreground process:
                                // kills it by default, runs the handler if
                                // the process registered one (miku_signal)
                                let fg = crate::io::input::user_stdin::foreground_pid();
                                if fg != 0 {
                                    crate::println!("^C");
                                    crate::process::signal::send_signal(fg, crate::process::signal::SIGINT);
                                }
                            }
                            DecodedKey::Unicode(c) => {
                                crate::io::input::user_stdin::feed_char(c);
                            }
                            DecodedKey::RawKey(_) => {}
                        }
                    } else {
        ...
        if !got {
            crate::sched::sleep(KBD_POLL_TICKS);
        }
    }
}
...
pub fn shell_thread() -> ! {
    ...
    loop {
        ...
        crate::shell::commands::ext_cmds_common::periodic_flush_check();
...
pub fn on_shell_restart() {
    ...
    crate::io::input::user_stdin::clear_foreground();
...
pub fn on_kbd_restart() {
    while crate::io::input::stdin::pop().is_some() {}
